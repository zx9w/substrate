...
impl OverlayedChanges {
	...
	pub fn value_mut_or_insert_with(
		...
		let entry = self.top.modify(key, self.transaction_depth, self.extrinsic_index());
	...
	pub(crate) fn set_child_storage(
		...
		let entry = map_entry.0.modify(&key, self.transaction_depth, self.extrinsic_index());
		entry.value = val;
	}
	...
	pub(crate) fn clear_child_storage(
		...
		for (key, _) in &changeset.changes {
			changeset.modify(key, self.transaction_depth, self.extrinsic_index()).value = None;
		}
	}
	...
	pub(crate) fn clear_prefix(&mut self, prefix: &[u8]) {
		for (key, _) in self.top.changes.iter().filter(|(key, _)| key.starts_with(prefix)) {
			self.top.modify(key, self.transaction_depth, self.extrinsic_index()).value = None;
		}
	}
