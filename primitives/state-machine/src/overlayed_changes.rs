...
impl OverlayedChangeSet {
	...
	fn modify(&mut self, key: &[u8], at_extrinsic: Option<u32>) -> &mut OverlayedValue {
		if self.last_written < self.transaction_depth {
			self.dirty_keys.push(Default::default());
			self.last_written = self.transaction_depth;
		}
		// There is no dirty set when no transaction is open.
		...
		if overlayed.last_written < self.transaction_depth {
			overlayed.value.push(Default::default());
			overlayed.extrinsics.push(Default::default());
			overlayed.last_written = self.transaction_depth;
		}
	...
	fn close_transaction(&mut self, commit: bool) {
		if let Some(depth) = self.transaction_depth.checked_sub(1) {
			self.transaction_depth = depth;
		} else {
			return;
		}

		for key in self.dirty_keys.pop().expect("transaction_depth was > 0") {
			let overlayed = self.changes.get(&key).expect("key was marked as dirty");
			let value = overlayed.value.pop().expect("key was marked dirty for this tx");

			if commit {
				*overlayed.value_mut() = value;
				self.last_written = self.transaction_depth;
				// for transaction_depth == 0 there is no dirty key set
	...
	fn start_transaction(&mut self) {
		self.transaction_depth.checked_add(1).unwrap();
	}
